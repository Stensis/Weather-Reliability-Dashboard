...
export function useDashboardController() {
  ...
  const useBrowserLocation = useCallback(() => {
    if (!navigator.geolocation) {
      setError(
        "Browser geolocation is not supported on this device. Search for a city instead.",
      );
      return;
    }
  ...
  const metrics = useMemo(() => {
    ...
    if (current.rainProbability !== undefined) {
      items.push({
        icon: CloudRain,
        label: "Rain chance",
        value: formatPercent(current.rainProbability),
        helper: "Precipitation probability",
      });
    }
    ...
    if (current.airQuality !== undefined) {
      items.push({
        icon: Gauge,
        label: "Air quality",
        value: String(current.airQuality),
